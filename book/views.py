...
def between(request):
    # data = BooksModel.objects.all().order_by('bookname')
    data = BookModel.objects.filter(price__lt=300) & BookModel.objects.filter(price__gt=100).order_by(
        'bookname').reverse()
    # data = BooksModel.objects.all().order_by(Coalesce('bookname','bookname').desc())
    # data = BooksModel.objects.all().order_by(Lower('bookname').desc())
...
def subject(request):
    ...
    if request.GET:
        ...
        data = BookModel.objects.filter(subject=subjectname)
    return render(request, "searchsubject.html", {'subject': subjectname, "title": "search", "books": data})
